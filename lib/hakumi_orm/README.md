...
| `expr.rb` | `Expr` (abstract, sealed) | Expression tree for WHERE clauses. Subclasses: `Predicate` (leaf node with field + operator + binds), `AndExpr`, `OrExpr`, `NotExpr`. Supports `and`, `or`, `not` composition with deterministic parentheses. Operator aliases: `&` (`and`), `\|` (`or`), `!` (`not`) -- all delegate to the named methods. |
...
| `relation.rb` | `Relation[ModelType]` (abstract, generic) | Fluent query builder with mutable chaining: `where`, `order`, `order_by`, `limit`, `offset`, `join`, `preload`. Subclasses implement `hydrate` to materialize rows. Terminal methods: `to_a`, `first`, `count`, `pluck_raw`, `delete_all`, `update_all`, `to_sql`. |
...
| `codegen/generator_validation.rb` | `Codegen::Generator` (reopened) | Validation and variant builder methods extracted from Generator to stay under the ClassLength limit: `build_checkable`, `build_validated_record`, `build_base_contract`, `build_contract`, `build_variant_base`, `build_validated_insert_locals`, `generate_contracts!`. |
