...
| `sql_compiler.rb` | `SqlCompiler` | Compiles `Expr` trees, ordering, joins, limit/offset into parameterized SQL. Generates `SELECT`, `INSERT`, `UPDATE`, `DELETE`, `EXISTS` with sequential bind markers. All values go through bind parameters, never interpolated. |
| `relation.rb` | `Relation[ModelType]` (abstract, generic) | Fluent query builder with mutable chaining: `where`, `order`, `order_by`, `limit`, `offset`, `join`, `preload`. Subclasses implement `hydrate` to materialize rows. Terminal methods: `to_a`, `first`, `count`, `exists?`, `pluck_raw`, `delete_all`, `update_all`, `to_sql`. |
