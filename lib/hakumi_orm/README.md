...
| "fixtures/loader.rb" | "Fixtures::Loader" | Loads YAML fixtures (with ERB support) into DB tables using typed bind encoding and table replacement semantics. Supports folder/file filtering equivalent to Rails task flags, FK label reference resolution, dependency ordering, and optional post-load FK integrity checks. |
