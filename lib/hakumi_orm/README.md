...
| "tasks.rb" | "Tasks" | Rake tasks for HakumiORM. "require "hakumi_orm/tasks"" adds: "db:install" (setup generator), "db:generate" (generate + annotate), "db:migrate", "db:rollback[N]", "db:migrate:status", "db:version", "db:migration[name]", "db:type[name]", "db:associations" (list all associations). |
...
| "record_runtime.rb" | "RecordRuntime" | Shared runtime helpers for generated records. Provides result hydration helpers (`hydrate_result_rows`, `hydrate_result_first`) and typed hash comparison helpers used by generated `changed_from?` / `diff`. |
