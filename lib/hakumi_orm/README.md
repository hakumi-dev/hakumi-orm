...
|---|---|---|
| "hakumi_orm.rb" | "HakumiORM" | Entry point. Provides "configure(&blk)", "config", "adapter(name = nil)", "adapter=", "using(name, &blk)", "reset_config!". "adapter" checks thread-local override first (set by "using"), then falls back to primary. All generated code defaults to "HakumiORM.adapter". |
