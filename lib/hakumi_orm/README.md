...
| "validation/contract_dsl.rb" | "Validation::ContractDSL" | Contract validation DSL backing "validates" (standard rules) and "validate" (custom methods) in generated "BaseContract". Integrates with lifecycle contexts ("all", "create", "update", "persist", "destroy"). |
...
| "configuration.rb" | "Configuration" | Global config object. Primary attributes: "adapter_name", "database", "host", "port", "username", "password", "output_dir", "models_dir", "contracts_dir", "module_name", "adapter", "pool_size", "pool_timeout", "logger" ("T.nilable(Loggable)"), "pretty_sql_logs", "colorize_sql_logs", "log_filter_parameters", "log_filter_mask", "migrations_path", "definitions_path", "seeds_path", "fixtures_path", "verify_foreign_keys_for_fixtures", "connection_options", "form_model_adapter" ("FormModelAdapter"), "database_url=". "log_level=" creates an internal "Logger" to "$stdout" with the given level (":debug", ":info", ":warn", ":error", ":fatal"). Multi-DB: "database_config(name, &blk)" registers named databases, "adapter_for(name)" lazily builds adapters, "named_database(name)" returns config, "database_names" lists names, "register_adapter(name, adapter)" for manual injection, "close_named_adapters!" cleanup. "connection_options" stores extra driver params (sslmode, connect_timeout, etc.) extracted from URL query params. Runtime adapter creation and boot checks are split into extension files to keep the core config object focused. |
...
| "application/fixtures_load.rb" | "Application::FixturesLoad" | Public application operation for fixtures. Exposes "load!", "load_with_data!", and "plan_load!" using config + adapter inputs, while keeping "Fixtures::Loader" as an internal implementation detail. |
...
| "fixtures/loader.rb" | "Fixtures::Loader" | Loads YAML fixtures (with ERB support) into DB tables using typed bind encoding and table replacement semantics. Supports folder/file filtering equivalent to Rails task flags, FK label reference resolution, dependency ordering, optional post-load FK integrity checks, and dry-run planning metadata for fixture analysis. |
| "fixtures/reference_resolver.rb" | "Fixtures::ReferenceResolver" | Resolves fixture FK references from labels, expands multi-label join-style rows, and computes dependency-safe table insertion order. |
| "fixtures/integrity_verifier.rb" | "Fixtures::IntegrityVerifier" | Verifies fixture FK integrity after load and reports orphan findings with per-FK details. |
...
| "codegen/definition_loader.rb" | "Codegen::DefinitionLoader" | Unified loader for codegen definitions. Clears registries, then loads either a single file (default "db/definitions.rb") or every "*.rb" in a directory, and returns both "{ associations:, enums: }". Used by Rake tasks. |
