...
| File | Module / Class | Description |
|---|---|---|
| "bind.rb" | "Bind" (abstract, sealed) | Base class for typed bind parameters. All subclasses co-located in this file for "sealed!": "IntBind", "StrBind", "FloatBind", "DecimalBind", "BoolBind", "TimeBind", "DateBind", "JsonBind", "NullBind", "IntArrayBind", "StrArrayBind", "FloatArrayBind", "BoolArrayBind". Abstract method "serialize" returns the adapter-neutral wire value (String or numeric). Zero "T.untyped" -- custom types reuse existing Bind subclasses (e.g., Money -> "DecimalBind"). Array binds serialize to PG array literal format ("{1,2,3}"). "StrArrayBind" always quotes non-NULL values to handle curly braces, newlines, tabs, and all edge cases. |
...
| "compiled_query.rb" | "CompiledQuery" | Immutable container holding a SQL string and its associated "T::Array[Bind]". Provides "db_params" to extract serialized values (adapter-neutral) and "params_for(dialect)" for dialect-encoded values with caching. |
